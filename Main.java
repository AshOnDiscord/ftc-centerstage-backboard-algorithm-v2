...
//    }
        ...
        Pixel[][] mosaics = newBoard.findMosaics();
        Arrays.stream(mosaics).filter(e -> Objects.nonNull(e[0])).forEach(mosaic -> {
        ...
        ArrayList<PixelData> newColors = newBoard.coloredColor(Arrays.stream(mosaics).filter(e -> Objects.nonNull(e[0])).collect(Collectors.toCollection(ArrayList::new)));
        newColors.forEach((e) -> {
            System.out.println(e);
            newBoard.pixels[e.y][e.x].color = e.color;
        });
...
class Board {
    public Pixel[][] pixels = new Pixel[11][];
    ...
    public void setup() {
        for (int i = 0; i < 11; i++) {
            this.pixels[i] = new Pixel[6 + (i % 2)];
            for (int j = 0; j < 6 + (i % 2); j++) {
                this.pixels[i][j] = new Pixel(this, i, j);
            }
        }
    }
    ...
    public void printBoard() {
        ...
        for (int i = this.pixels.length - 1; i >= 0; i--) {
            Pixel[] row = this.pixels[i];
            StringBuilder line = new StringBuilder(row.length % 2 == 0 ? " " : "");
            // row.forEach(pixel -> {
            for (Pixel pixel : row) {
    ...
    public Pixel[][] findMosaics() {
        ArrayList<Pixel> checked = new ArrayList<>();
        Pixel[][] mosaics = new Pixel[10][3];
        int index = 0;
        for (Pixel[] pixelArrayList : this.pixels) {
...
//                      mosaics.add(new Pixel[]{pixel, coloredNeighbors.get(0), coloredNeighbors.get(1)});
                        mosaics[index][0] = pixel;
                        mosaics[index][1] = coloredNeighbors.get(0);
                        mosaics[index][2] = coloredNeighbors.get(1);
                        index++;
                        continue;
                    }
...
//                    mosaics.add(new Pixel[]{pixel, sameColor.get(0), sameColor.get(1)});
                    mosaics[index][0] = pixel;
                    mosaics[index][1] = sameColor.get(0);
                    mosaics[index][2] = sameColor.get(1);
                    index++;
                }
            }
        }
    ...
    public int getLines() {
        int lines = 0;
        for (int i = 2; i < this.pixels.length; i += 3) {
            if (Arrays.stream(this.pixels[i]).noneMatch(p -> p.color != Color.empty))
    ...
    public int getScore() {
        ...
        Pixel[][] mosaics = this.findMosaics();
        score += (int) (Arrays.stream(mosaics).filter(e ->Objects.nonNull(e[0])).count() * 10);
    ...
    public Board copy() {
        ...
        copy.pixels = Arrays.stream(this.pixels)
                .map(row -> Arrays.stream(row)
                        .map(p -> new Pixel(copy, p.y, p.x, p.color)).toArray(Pixel[]::new))
                .toArray(Pixel[][]::new);
        return copy;
    }
...
class Pixel extends PixelData {
    ...
    public Pixel[] getNeighbors(NeighborType type) {
        ...
        for (int i = 0; i < offsets.length; i++) {
            ...
            if (newY < 0 || newY >= this.board.pixels.length || newX < 0 || newX >= this.board.pixels[newY].length) {
                continue;
            }
            Pixel pixel = this.board.pixels[newY][newX];
